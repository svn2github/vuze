...
public final class SHA1Az {
  ...
  public byte[] digest(ByteBuffer buffer) {
    ...
    int totalLength = 64 * ((length2 + 63) / 64);
    ...
    buffer.position(length2);
    int needsFill = totalLength - 9 - length2;
    if(needsFill >= 0) {
      buffer.put((byte)0x80);
      for(int i = 0 ; i < needsFill ; i++) {
       buffer.put((byte)0);       
      }
      buffer.putLong((length2 << 3));     
    } else {
      finalBuffer.position(0);
