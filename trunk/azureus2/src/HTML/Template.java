 ...
 *
 *      Modified by Tobias Minich, Aug 2003
 *      Unmodified form is available from http://html-tmpl-java.sourceforge.net
 */
...
public class Template {
  ...
  public List vars = new LinkedList();
  ...
  throws FileNotFoundException {
      ...
      //return new BufferedReader(new FileReader(filename));
    {
      if (filename.startsWith("RES:")) {
        InputStream is = ClassLoader.getSystemResourceAsStream(filename.substring(filename.indexOf(':')+1));
        if (is == null)
    ...
    for(int i=0; i<this.path.length; i++) {
      try {
        ...
        if (this.path[i].startsWith("RES:")) {
          InputStream is = ClassLoader.getSystemResourceAsStream(this.path[i].substring(this.path[i].indexOf(':')+1) + "/" +filename);
          if (is != null)
