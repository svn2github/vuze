					...
					display.asyncExec(new Runnable() {
						public void run() {
							if (!mainWindow.isDisposed()
								...
								Tab.refresh();
								statusDown.setText(
									"D: " + globalManager.getDownloadSpeed());
								statusUp.setText(
									"U: " + globalManager.getUploadSpeed());
							}
							if(trayIcon != null)
							trayIcon.refresh();
							synchronized (downloadBars) {
								Iterator iter =
									downloadBars.values().iterator();
								while (iter.hasNext()) {
									MinimizedWindow mw =
										(MinimizedWindow) iter.next();
									mw.refresh();
								}
							}
						}
		...
		file_new_folder.addListener(SWT.Selection, new Listener() {
			public void handleEvent(Event e) {
				DirectoryDialog fDialog =
					new DirectoryDialog(mainWindow, SWT.NULL);
				fDialog.setText(
					"Choose the directory containing the torrent files");
				String fileName = fDialog.open();
				if (fileName == null)
					return;
	...
	public void openTorrent(String fileName) {
		...
		display.asyncExec(new Runnable() {
			public void run() {
				...
				DirectoryDialog dDialog =
					new DirectoryDialog(mainWindow, SWT.NULL);
				dDialog.setFilterPath(
					ConfigurationManager.getInstance().getStringParameter(
						"Default Path",
						""));
				...
				globalManager.addDownloadManager(
					new DownloadManager(globalManager, _fileName, savePath));
			}
		});
	}
