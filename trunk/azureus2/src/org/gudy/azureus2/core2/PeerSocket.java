...
public class PeerSocket extends PeerConnection {
  ...
  protected void write() { //If we are already sending something, we simply continue
    ...
    if (writeBuffer == null) { //So the ByteBuffer is null ... let's find out if there's any data in the protocol queue
      ...
      if (dataQueue.size() > 0) {        
        ...
        if (!choking) {
          ...
          if (item.isLoaded()) {
            ...
            logger.log(componentID,evtProtocol,Logger.SENT,ip + " is being sent #" + request.getPieceNumber() + ":" + request.getOffset() + "->" + (request.getOffset()+request.getLength()));
            // Assign the current buffer ... 
            keepAlive = 0;
