	...
	implements PEPeerTransport
{
  ...
  private void decodeBTHandshake( BTHandshake handshake ) {
    ...
    if( maxAllowed == 0 ) {
      closeConnectionInternally( "too many existing peer connections [" +PeerIdentityManager.getIdentityCount( my_peer_data_id )+ " / " +maxAllowed+ "]" );
      handshake.destroy();
      return;
    }
  ...
  private void decodeAZPeerExchange( AZPeerExchange exchange ) {
    ...
    //make sure they're not spamming us
    if( !message_limiter.countIncomingMessage( exchange.getID(), 6, 220*1000 ) ) {  //allow max 5 pex per 4min  //TODO reduce to 5 max after 2306 release
