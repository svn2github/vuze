	...
	private void checkCompletedPieces() {
		...
		for (int i = 0; i < nbPieces; i++) {
			...
			if (null !=piece && piece.isComplete() && !piece.isBeingChecked())
			{
				...
				_diskManager.enqueueCheckRequest( _diskManager.createCheckRequest(i, new Boolean(false)), this );	
			}
		}
	}
    ...
    if (all_pieces_done) {
      ...
      //remove previous snubbing
			...
			if (checkPieces && !start_of_day) {
      			...
      			_diskManager.enqueueCompleteRecheckRequest( _diskManager.createCheckRequest( -1, new Boolean( true )), this );
			}
...
//	piece from a torrent
	 ...
	 * @return true if a request was assigned, false otherwise
	 */
	public int[] getRarestPieceInfo(PEPeer pc)		//PEPeerTransport pc)
	{
		if (null ==pc)
			return null;
		int i;
			...
			if (null !=dm_pieces[i] &&!dm_pieces[i].getCompleted() &&piecesAvailable[i] &&!_downloading[i])
			{
				...
				if (0 <=priority)
				{
					...
					if (null !=_pieces[i])
					{
						...
						if (avail <ResumeMinAvail ||(!RarestCanResume &&((avail -(priority /1001)) <=ResumeMinAvail) &&priority >ResumeMaxPriority))
						{
...
//	Returns time started in ms
  ...
  processPieceCheckResult(
	...
	int						outcome )
  {
  	boolean	recheck_on_completion = ((Boolean)request.getUserData()).booleanValue();
