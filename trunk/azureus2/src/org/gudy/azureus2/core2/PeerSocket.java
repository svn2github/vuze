...
public class PeerSocket extends PeerConnection {
	...
	public PeerSocket(
		...
		logger.log(
			...
			evtLifeCycle,
			Logger.INFORMATION,
			"Creating outgoing connection to " + ip + " : " + port);
	...
	public PeerSocket(PeerManager manager, SocketChannel sck) {
		...
		logger.log(
			...
			evtLifeCycle,
			Logger.INFORMATION,
			"Creating incoming connection from " + ip + " : " + port);
		handShake();
	...
	public synchronized void closeAll() {
		...
		try {
			...
			logger.log(
				...
				evtErrors,
				Logger.ERROR,
				"Error in PeerConnection::closeAll-sck.close() ("
					+ ip
					+ " : "
					+ port
					+ " ) : "
		...
		public void process() {
			try {
				...
				logger.log(
					...
					evtErrors,
					Logger.ERROR,
					"Error in PeerConnection::initConnection ("
						+ ip
						+ " : "
						+ port
						+ " ) : "
	...
	private void analyseBuffer(ByteBuffer buffer) {
		...
		switch (cmd) {
			...
			case BT_REQUEST :
				...
				if (manager
					.checkBlock(pieceNumber, pieceOffset, pieceLength)) {
					sendData(
						new Request(pieceNumber, pieceOffset, pieceLength));
				} else {
					logger.log(
						componentID,
						evtErrors,
			...
			case BT_PIECE :
				...
				if (requested.contains(request)
					&& manager.checkBlock(pieceNumber, pieceOffset, buffer)) {
	...
	public void sendHave(int pieceNumber) {
		...
		logger.log(
			...
			evtProtocol,
			Logger.SENT,
			ip + " is notified you have " + pieceNumber);
	...
	private void sendBitField() {
		ByteBuffer buffer =
			ByteBuffer.allocate(5 + (manager.getPiecesNumber() + 7) / 8);
		...
		if (writeBuffer == null) {
			...
			if (dataQueue.size() != 0) {
				...
				if (!choking) {
					...
					if (item.isLoaded()) {
						dataQueue.remove(item);
						ByteBufferPool.getInstance().freeBuffer(
							item.getBuffer());
					}
					return;
				}
