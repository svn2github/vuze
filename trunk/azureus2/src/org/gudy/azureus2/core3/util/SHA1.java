...
public final class SHA1 {
	...
	public void update(ByteBuffer buffer) {
		...
		{
			int toProcess = buffer.arrayOffset()+buffer.limit()-buffer.limit()%64;
			for(int i = buffer.arrayOffset()+buffer.position();i < toProcess;i+=64)
				transform(buffer.array(),i);
			buffer.position(buffer.limit()-buffer.limit()%64);
		}
