...
DownloadManagerController 
{
		...
		TRTrackerAnnouncer	tracker_client ) 
	{
		...
		if ( peer_manager != null ){
			...
			Debug.out( "Peer manager not null" );
		}
		
			// make sure it is started beore making it "visible"
		
		PEPeerManager temp = PEPeerManagerFactory.create( download_manager, tracker_client, disk_manager);
		...
		int	initialising_state ) 
	{
		...
		setState( initialising_state );
	  	...
	  	DiskManager	res = disk_manager;
	  	  	
	  	if ( res == null ){
	  	  		
  	  		res = DiskManagerFactory.create( download_manager.getTorrent(), download_manager);
  	      
  	  		setDiskManager( res );
  	  		

  	  		
  	  		disk_manager.addListener( disk_manager_listener );
  	  	}
	  	    ...
	  	    Thread recheck = 
	  	    	new AEThread("forceRecheck") 
	  			{
	  				public void runSupport(){
	  					...
	  					DiskManager recheck_disk_manager = initializeDiskManager( DownloadManager.STATE_CHECKING );
	  					...
	  					if ( recheck_disk_manager.getState() == DiskManager.READY ){
	  					  	...
	  					  	try{
	  							...
	  							download_manager.setOnlySeeding(recheck_disk_manager.getRemaining() == 0);
				...
				run()
				{
					...
					try{
						...
						if ( disk_manager != null ){
							...
							disk_manager.removeListener( disk_manager_listener );
